...
public class InicializarDatos implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		for (int i = 0; i < 100; i++) {
			...
			jabon.setTipoDePiel(TipoDePiel.values()[faker.random().nextInt(TipoDePiel.values().length)]); // Asume que
																											// tienes
																											// una
																											// enumeración
																											// TipoDePiel
		...
		// Crear algunas categorías ficticias
		Categoria categoria1 = new Categoria("Cuidado Facial", "Productos para el cuidado facial");
		Categoria categoria2 = new Categoria("Cuidado Corporal", "Productos para el cuidado corporal");
		...
		for (int i = 0; i < 3; i++) {
			...
			producto.setPrecio(Double.parseDouble(faker.commerce().price().replaceAll("[^\\d.]+", "")));
			producto.setDescripcion(faker.lorem().sentence());
			producto.setStock(faker.number().numberBetween(0, 100));
			producto.setImagenUrl(generarUrlImagenAleatoria());

			// Asociar categorías aleatorias al producto
	...
	private Ingrediente generarIngredienteFicticio(Faker faker) {
		...
		String[] elementos = { "jabón de glicerina", "gel aloe vera", "miel", "aceite de oliva", "ralladura de limón",
				"aceite esencial" };
